	...
	Dashboard.Projects.Views.Project = Backbone.View.extend({
		...
		render: function(done)
		{
			...
			if(_.isNull(this.model.get('id'))) blanks['view_url']= sessionStorage.getItem('hostname') + sessionStorage.getItem('directory') + this.model.id;
			...
			var html =
						'<div class="row">'+
							'<div id="zeega-embed" class="span7 project-image" style="background:url(<%= cover_image %>) no-repeat center center;background-size:cover">'+
								...
								if (zeega.dashboard.app.editable && this.model.get('editable') == true)
								{
									html+=
