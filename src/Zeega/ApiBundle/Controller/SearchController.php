...
class SearchController extends Controller
{
		...
		*/
		...
		if($solrEnabled)
		{
			if(!$returnCollections)
				...
				return $this->searchCollectionsWithDoctrine();
		}
		else
        {
        	//return "yo";
	 ...
	 *
     */
    private function searchCollectionsWithDoctrine()
    {
        $user = $this->get('security.context')->getToken()->getUser();
	    $request = $this->getRequest();
	    $userId        = $request->query->get('user');      //  int
	    ...
	    $collections = $this->getDoctrine()->getRepository('ZeegaDataBundle:Item')->findByUserSiteMediatype($userId,$siteId,'Collection');
	    $results['collections'] = $collections;
