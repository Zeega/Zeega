		...
		if($widgetId) {
			...
			if(!$thumbUrl||$img==FALSE){
				...
				elseif($item->getContentType()=='Audio'){
					@$img=file_get_contents($this->container->getParameter('thumbnails_uri') . '/templates/audio.jpg');
				
				}
				elseif($item->getContentType()=='Video'){
					@$img=file_get_contents($this->container->getParameter('thumbnails_uri') . '/templates/video.jpg');
